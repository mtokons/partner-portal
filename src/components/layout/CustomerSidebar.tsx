      ...
      <div className="p-6 border-b border-sidebar-border">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-sidebar-foreground/60 uppercase tracking-widest">My Services</p>
          </div>
        </div>
      </div>
        ...
        {navLinks.map((link) => {
          ...
          return (
            ...
            >
              ...
              {link.label}
              {link.href === "/customer/notifications" && unreadCount > 0 && (
                <span className="ml-auto bg-teal-500 text-white text-[10px] rounded-full px-1.5 py-0.5 min-w-[1.25rem] text-center font-bold">
                  {unreadCount}
                </span>
              )}
              {isActive && (
