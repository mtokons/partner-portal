...
export default function Header({ userName, company, overdueCount, unpaidInvoicesCount }: HeaderProps) {
  ...
  return (
    <header className="fixed top-0 left-64 right-0 z-30 h-16 header-glass flex items-center justify-between px-6">
      {/* Left: page context / search */}
      <div className="flex items-center gap-4">
        <div className="hidden lg:block">
          <p className="text-[11px] text-muted-foreground/70 uppercase tracking-[0.12em] font-medium leading-none">
            {company}
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className={cn(
              ...
              searchFocused
                ? "w-72 border-primary/40 bg-white ring-2 ring-primary/10 shadow-md"
                : "w-52 border-border hover:border-primary/20 hover:bg-muted/60"
            )}
          />
          <kbd className="absolute right-2.5 top-1/2 -translate-y-1/2 hidden sm:flex items-center gap-0.5 text-[10px] text-muted-foreground/40 font-mono">
            ⌘K
          </kbd>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <Button
          variant="ghost"
          size="sm"
          className="h-9 w-9 rounded-xl p-0 text-muted-foreground/60 hover:text-foreground hidden sm:flex"
        >
        ...
        <Button
          ...
          className="h-9 w-9 rounded-xl p-0 text-muted-foreground/50 hover:text-destructive hover:bg-red-50 transition-all ml-1"
          title="Sign out"
        >
