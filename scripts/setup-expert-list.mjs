...
async function run() {
  try {
    ...
    // Create Experts List
    await createListIfNotExists(siteId, "Experts", [
      { name: "Name", text: {} },
      { name: "Email", text: {} },
      { name: "Phone", text: {} },
      { name: "Specialization", text: {} },
      { name: "Bio", text: { allowMultipleLines: true } },
      { name: "Status", choice: { choices: ["active", "inactive", "on-leave"] } },
      { name: "Rating", number: {} },
      { name: "TotalSessionsCompleted", number: {} },
      { name: "RatePerSession", currency: { locale: "de-DE" } },
      { name: "FirebaseUid", text: {} }, // Connector to Auth
      { name: "CreatedAt", dateTime: {} }
    ]);

    console.log("✅ Experts list provisioned successfully!");
  } catch (error) {
