...
STAGE_DIR="/tmp/partner-portal-deploy-stage"
...
echo "📦 Staging build artifacts to ${STAGE_DIR} (outside OneDrive sync)..."
rm -rf "${STAGE_DIR}"
...
tar -czf /tmp/standalone.tar.gz -C "${STAGE_DIR}" standalone
...
tar -czf /tmp/static.tar.gz -C "${STAGE_DIR}" static
...
tar -czf /tmp/public.tar.gz -C "${STAGE_DIR}" public
